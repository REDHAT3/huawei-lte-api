...
class Connection:
    ...
    def __init__(
        ...
        adapter = HTTPAdapterWithSocketOptions(
            socket_options=[(socket.SOL_SOCKET, 25, iface.encode("utf-8"))]
        )
    ...
    @staticmethod
    def _create_request_xml(
        data: Union[dict, list], dicttoxml_xargs: Optional[dict] = None
    ) -> bytes:
    ...
    def _process_response_xml(response: requests.Response) -> dict:
        ...
        try:
            ...
            if response.history:
                return dict(
                    error=dict(
                        code=ResponseCodeEnum.ERROR_SYSTEM_NO_SUPPORT, message=""
                    )
                )
            raise
    ...
    def _check_response_status(data: dict) -> Union[dict, str]:
        ...
        if "error" in data:
            ...
            raise error_code_to_exception.get(error_code, ResponseErrorException)(
                "{}: {}".format(error_code, message), error_code
            )
    ...
    def _post(
        ...
        if self.request_verification_tokens:
            if len(self.request_verification_tokens) > 1:
                ...
                ] = self.request_verification_tokens.pop(0)
            else:
                headers[
                    "__RequestVerificationToken"
                ] = self.request_verification_tokens[0]
        ...
        response_data = cast(
            str, self._check_response_status(self._process_response_xml(response))
        )
        ...
        if "__RequestVerificationTokenone" in response.headers:
            self.request_verification_tokens.append(
                response.headers["__RequestVerificationTokenone"]
            )
            if "__RequestVerificationTokentwo" in response.headers:
                self.request_verification_tokens.append(
                    response.headers["__RequestVerificationTokentwo"]
                )
        elif "__RequestVerificationToken" in response.headers:
            self.request_verification_tokens.append(
                response.headers["__RequestVerificationToken"]
            )
        else:
    ...
    @_try_or_reload_and_retry
    def get(
        self, endpoint: str, params: Optional[dict] = None, prefix: str = "api"
    ) -> dict:
        headers = {}
        if len(self.request_verification_tokens) == 1:
        ...
        return cast(
            dict, self._check_response_status(self._process_response_xml(response))
        )
